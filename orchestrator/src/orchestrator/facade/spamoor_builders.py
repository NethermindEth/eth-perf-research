"""Vendored from ethereum/execution-specs feat/spamoor-to-est @ 8d89e82f8.

Source: tests/benchmark/spamoor/helpers.py.

Pytest-only helpers (``spamoor_signer_context``, ``broadcast_and_assert_receipts``,
the ``import pytest`` and ``execution_testing.test_types`` paths inside them) and
unused EELS-internal imports have been stripped so this module is importable in
the orchestrator's runtime environment without pulling EELS into the dependency
tree. The 12 ``build_*_transactions`` functions are kept byte-for-byte so a
future re-sync is ``cp`` + manual diff.
"""
